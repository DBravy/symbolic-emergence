...
from retrieval_decoder import build_retrieval_decoder
from agent import GradientScaleHook, GradientAttenuationHook

class RetrievalProgressiveAgent(nn.Module):
    """
    Progressive agent with retrieval capability for constrained puzzle selection
    """
    def __init__(
        self, 
        agent_id: str,
        ...
        # Use retrieval decoder instead of regular decoder
        self.decoder = decoder if decoder is not None else build_retrieval_decoder(
            embedding_dim=embedding_dim,
            hidden_dim=hidden_dim,
            puzzle_symbols=puzzle_symbols,
    ...
    def set_puzzle_database(self, puzzle_database: List[torch.Tensor], puzzle_embeddings: torch.Tensor):
        """Set the puzzle database for retrieval mode"""
        if hasattr(self.decoder, 'set_puzzle_database'):
            self.decoder.set_puzzle_database(puzzle_database, puzzle_embeddings)
        else:
            print(f"Warning: {self.agent_id} decoder does not support retrieval mode")
    
    def set_retrieval_mode(self, enabled: bool):
        """Enable or disable retrieval mode"""
        if hasattr(self.decoder, 'set_retrieval_mode'):
            self.decoder.set_retrieval_mode(enabled)
        else:
            print(f"Warning: {self.agent_id} decoder does not support retrieval mode")
    
    def get_retrieval_stats(self) -> dict:
        """Get retrieval statistics"""
        if hasattr(self.decoder, 'get_retrieval_stats'):
            return self.decoder.get_retrieval_stats()
        else:
            return {'retrieval_mode': False, 'database_size': 0}
    ...
    def gumbel_softmax(self, logits: torch.Tensor, temperature: float = 1.0, hard: bool = False) -> torch.Tensor:
    ...
    def encode_puzzle_to_message(self, puzzle_grid, temperature=1.0, initial_phase=False):
        ...
        # Apply position-based masking for symbol selection
        masked_symbols = []
        final_symbol_logits = []
        
        for pos in range(seq_len):
            # Get position mask (which symbols are allowed at this position)
            ...
            # Apply Gumbel-Softmax to masked logits
            pos_symbols = self.gumbel_softmax(masked_logits, temperature, hard=True)
    ...
    def decode_message_to_puzzle(   
        ...
        # Get decoder outputs (supports both retrieval and generation modes)
    ...
    # Rest of the methods remain the same as ProgressiveAgent
    def process_arc_example(self, input_grid: torch.Tensor, output_grid: Optional[torch.Tensor] = None, temperature: float = 1.0) -> Tuple[torch.Tensor, torch.Tensor, float]:
    ...
    def evaluate_communication(self, original_puzzle: torch.Tensor, reconstructed_puzzle: torch.Tensor) -> torch.Tensor:
...
# Factory function to create RetrievalProgressiveAgent instances
def RetrievalAgent(*args, **kwargs):
    """Factory function to create RetrievalProgressiveAgent instances"""
    return RetrievalProgressiveAgent(*args, **kwargs)